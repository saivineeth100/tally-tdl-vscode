...
export function provideSignatureHelp(
    ...
    if (foundDollar && functionNameStr) {
        ...
        if (func) {
            const parameters: ParameterInformation[] = (func.parameters || []).map((p: any) => ({
                label: p.ParameterType || 'Parameter',
                documentation: p.DataType ? `Data Type: ${p.DataType}` : undefined
            }));
            
            const signature: SignatureInformation = {
                label: `$$${func.name}${func.parameters && func.parameters.length > 0 ? ':' + func.parameters.map((p: any) => p.ParameterType).join(':') : ''}`,
    ...
    for (let i = parts.length - 2; i >= 0; i--) {
        ...
        if (action) {
            const parameters: ParameterInformation[] = (action.parameters || []).map((p: any) => ({
                label: p.ParameterType || 'Parameter',
                documentation: p.IsConstant ? `Constant. Type: ${p.ParameterType}` : undefined
            }));
            
            const signature: SignatureInformation = {
                label: `Action: ${action.name}${action.parameters && action.parameters.length > 0 ? ':' + action.parameters.map((p: any) => p.ParameterType).join(':') : ''}`,
